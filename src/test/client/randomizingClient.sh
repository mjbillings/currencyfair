...
#	Script to simulate client message producers
#	Multiple instances can be run to 
#	Randomizes user, to/from currency, buy/sell/rate
#	Sleeps random number of seconds to simulate real-live traffic

clear

declare -a userIDArray=(67844 59222 45030 33411 68987 56733 12990)
declare -a currencyArray=(EUR GBP USD SEK JPY CNY CRC AUD)
declare -a countryArray=(FR CA CN DK DE MX NO SK)
...
#	echo curl -i -H "Accept: application/json" -H "Content-Type:application/json" -X POST --data '$json' "http://localhost/currencyfair/tradeService.php"
	curl --silent -i -H "Accept: application/json" -H "Content-Type:application/json" -X POST --data "$json" "http://localhost/currencyfair/tradeService.php"
